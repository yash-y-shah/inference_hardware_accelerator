...
module im2col #( 
  ...
  assign inc_kernel_next = window_select[win_ctr]; 
  assign zero_inj_next = pad_region_next || !inc_kernel_next;
  
  reg [1:0] state;
  localparam IDLE = 2'b00, PROCESS = 2'b01, DONE = 2'b10, UNUSED_STATE = 2'b11; 

  // strict signed casting prevents unsigned underflow errors
  wire signed [$clog2(ROW*COL)+1:0] initial_addr;
  wire signed [$clog2(ROW*COL)+1:0] k_offset;
  
  assign initial_addr = ($signed(start_row_idx * STRIDE) - $signed(PADDING)) * $signed(COL) + ($signed(start_col_idx * STRIDE) - $signed(PADDING));
  
  // calculate the memory offset required to start in the middle of a kernel for Pass 2
  assign k_offset = ($signed(k_row_start) * $signed(COL)) + $signed(k_col_start);
                                 
  always @(*) begin
    if(state == PROCESS) begin
      logic signed [$clog2(ROW + 2*PADDING)+1:0] current_padded_row_abs;
      logic signed [$clog2(COL + 2*PADDING)+1:0] current_padded_col_abs;
      
      current_padded_row_abs = $signed(op_row_idx * STRIDE) + $signed(win_row_ctr);
      current_padded_col_abs = $signed(op_col_idx * STRIDE) + $signed(win_col_ctr);

      if ( (current_padded_row_abs >= $signed(PADDING)) && (current_padded_row_abs < $signed(ROW + PADDING)) &&
           (current_padded_col_abs >= $signed(PADDING)) && (current_padded_col_abs < $signed(COL + PADDING)) ) begin
